...
distribucionS <- function(s) {
  if(s == 0) {
    ...
    resultado <- 1 - exp(-s/5)*sum(choose(10,1:10)*0.2^(1:10)*(1-0.2)^(10-(1:10))*sapply(0:(10-1), function(x) sum(((s/5)^(0:x))/factorial((0:x)))))
  }
  return(resultado)
}
...
gFuncion <- function(k, lambda, f=fFuncion) {
  if(k == 0) {
    ...
    resultado <- (lambda/k)*sum(1:k*sapply(1:k,f)*sapply((k-(1:k)), gFuncion, lambda=lambda))
  }
  return(resultado)
}
...
dFuncionS <- function(x){
  if(x == 0) {
    ...
    resultado <- sum((1/3 + 5*(1:min(4,x))/(3*x) ) * sapply(1:min(4,x),fFuncion) * sapply(x - 1:min(4,x),dFuncionS))
  }
  return(resultado)
}
...
kableExtra::kable(tibble, "latex", booktabs = TRUE, digits = 4, caption = "Probabilidad acumulada de la distribución de pérdidas agregadas")
