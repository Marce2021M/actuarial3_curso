...
$$ F(x) = 1 - e^{\left( -\left( \frac{x}{\theta} \right)^{\gamma} \right)} \quad \text{para} \quad x \geq 0 $$

```{=tex}
\begin{enumerate}
    \item[a)] Utilice el método de momentos para estimar \( \gamma \) y \( \theta \). [10]
    \item[b)] Utilice el método de percentiles para estimar \( \gamma \) y \( \theta \). [10]
\end{enumerate}
```
```{r}
library(rootSolve)
...
# Initial estimates (modify based on intuition or prior knowledge)
initial_estimates <- 20

# Find the root
results <- uniroot(f = eq_system, start = initial_estimates)

# Display results
cat("Roots are:", results$root, "\n")
```
