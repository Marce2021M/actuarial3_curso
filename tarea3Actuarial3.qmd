...
b)  A partir de los datos agrupados, ajuste por máxima verosimilitud la Distribución Exponencial Inversa con función de distribución acumulada $F(x) = e^{-\frac{\theta}{x}} I(0,\infty)(x), \quad \theta > 0$. Especifique $\hat{\theta}$ y grafique la densidad ajustada junto con el histograma del inciso anterior. \[10\]
...
b)  Calcule los EMV de $\alpha$ y $\theta$. (Sugerencia: Recuerde que en el Ejercicio E14 se obtuvieron estos estimadores o utilice EViews para obtenerlos). \[5\]
...
```
  ...
  $$P(N = 0) = `r probabilidad1`$$

  $$P(N - E[N] \leq \sigma)= `r probabilidad2`$$
...
Con lo cual obtenemos lo siguiente
  ...
  $$P(N = 0) = `r probabilidad4`$$

  $$P(N - E[N] \leq \sigma)= `r probabilidad3`$$

Y el error de aproximación a cada son los siguientes:

$$\text{error} (P(N = 0))=`r error1`$$


$$\text{error}(P(N - E[N] \leq \sigma))=`r error2`$$
  ...
  En este caso contruimos $N=\sum_{i=1}^{m} N_{i}$ donde $N_{i}\sim \textbf{Geo}(p)$ con lo cual por el TCL tenemos:

  $$\frac{\sum_{i=1}^{m}N_{i}-m\frac{1-p}{p}}{\sqrt{m\frac{1-p}{p^{2}}}}\dot{\sim} N(0,1)$$
...
Con lo cual obtenemos lo siguiente
  ...
  $$P(N = 0)\approx P(N \leq 0 + .5) - P(N \leq 0 - .5)= `r probabilidad6`$$

  $$P(N - E[N] \leq \sigma) \approx P(N \leq \sigma + E[N]+.5)$$ 
  
  $$=`r probabilidad5`$$

donde $N\dot{\sim} N(E[N],Var(N))$

  Y los errores de aproximación fueron los siguientes:

  $$\text{error} (P(N = 0))=`r abs(probabilidad1-probabilidad6)`$$

  $$\text{error}(P(N - E[N] \leq \sigma))=`r abs(probabilidad2-probabilidad5)`$$
...
$$\frac{p_{2}}{p_{1}}=a+\frac{b}{2}$$

$$\frac{p_{3}}{p_{2}}=a+\frac{b}{3}$$
...
Con lo cual tenemos que $a=`r X[1]`$ y $b=`r X[2]`$. y
